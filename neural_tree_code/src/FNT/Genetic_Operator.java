...
public class Genetic_Operator {
    ...
    FNT[] geneticOperators(MersenneTwisterFast m_Random, FNT[] pool, double MUTATION_PROB, ArrayList treeParams) {
        m_RNG = m_Random;
                ...
                if (uniqueParent) {
                    ...
                    if (!newTree.isEmpty()) {
                        for (Object newTree1 : newTree) {
                            //System.out.println(indx + " C " + (FNT)newTree1);
                            child.add((FNT) newTree1);
                            indx++;
                        } //for                
                    } else {
        ...
        FNT[] newPopulation = new FNT[offspringLength];
        for (int i = 0; i < child.size(); i++) {
            newPopulation[i] = (FNT) child.get(i);
        }//for
    ...
    private Vector crossover(FNT first, FNT second) {
    ...
    private void mutationOneLeaf(FNT tree) {
        try {
    ...
    private void mutationAllLeafs(FNT tree) {
        try {
    ...
    private void mutationPruning(FNT tree) {
        try {
    ...
    private void mutationGrowing(FNT tree) {
        try {
            ////System.out.print("\n mutationGrowing:");
            int maxDepth = 2;
            ...
            parent.removeAndGrow(node, maxDepth, tree.m_InputsCount, tree.m_MaxArity, treeParameters);
        } catch (Exception e) {
            //System.out.print("mutationGrowing-" + e);
        }
    }//mutationGrowing end

    private boolean isequal(FNT[] tree, int parent1, int parent2) {
        boolean equal = true;
    ...
    private FNT reproduction(FNT indv) {
    ...
    FNT[] newPopulation(MersenneTwisterFast m_Random, FNT[] x, ArrayList gpParameters, ArrayList treeParams) {
        m_RNG = m_Random;
        ...
        //FNT[] matingPool = new FNT[pool_size];

        //Until the mating pool is filled, perform tournament selection
        int i = 0;//ppol index
        ArrayList child = new ArrayList();
        FNT[] sortedPOP = sorting_SO_Population(x);
