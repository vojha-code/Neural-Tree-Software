...
public class MiscOperations {
    ...
    public void displaySO(FNT[] tree) {
        int i = 0;
        for (FNT tree1 : tree) {
            System.out.print((i++) + ": ");
            System.out.print(tree1);
    ...
    public FNT[] sorting_SO_Population(FNT[] tree) {
        try {
    ...
    FNT[] mergePopulation(FNT[] mainTree, FNT[] offspringTree) {
        int mN = mainTree.length;
        int N = mN + offspringTree.length;

        FNT[] newTree = new FNT[N];
        for (int i = 0; i < N; i++) {
            if (i < mN) {
    ...
    FNT[] nonDominationSort(FNT[] x) {
        ...
        FNT[] sorted_based_on_fronts = sort2DArray(x, index_of_Fronts);
        ...
        for (int i = 0; i < F.size() && !((Vector) F.get(i)).isEmpty(); i++) {
            ...
            for (int j = 0; j < obj; j++) {
                ...
                FNT[] sorted_based_on_objectives = sort2DArray(y, index_of_obj);
                double f_max = 0.0;
                double f_min = 0.0;
                if (j == 0) {
    ...
    private FNT[] sort2DArray(FNT[] array, int[] index) {
        int N = array.length;
        FNT[] sortedArray = new FNT[N];
        for (int i = 0; i < N; i++) {
    ...
    FNT[] replace_chromosome_SO(FNT[] tree, int pop) {
        int N = tree.length;
        int obj = 2;
        FNT[] replaced_pop = new FNT[pop];
        //copy rank
        ...
        FNT[] sorted_based_on_fitness = sort2DArray(tree, fitness_index);
    ...
    FNT[] replace_chromosome(FNT[] tree, int pop) {
        int N = tree.length;
        int obj = 2;
        FNT[] replaced_pop = new FNT[pop];
        //copy rank
        ...
        FNT[] sorted_based_on_ranks = sort2DArray(tree, rank_index);
        //System.out.println("Rank based Sorting :Replace");
        //display(sorted_based_on_ranks);
        for (int i = 0; i < N; i++) {
        ...
        for (int i = 0; i <= max_rank; i++) {
            ...
            if (current_index > pop) {
                ...
                FNT[] temp_pop = new FNT[temp_length];
                int idx_temp = 0;
    ...
    private boolean isequal(FNT Tree1, FNT Tree2) {
        boolean equal = true;
    ...
    FNT[] replaceWithBestDistinct(FNT[] tree, int pop) {//FNT array and number of candidates to be selected
        //int[] indexF = FramesCall.selectIndividuals(tree, pop);//it is here for just verification and has no use in this function
        ...
        for (int i = 1; i < N; i++) {
            if (indexBest < pop) {
                replaced_pop[indexBest] = sorted_based_on_fitness[indexBest].copyTree();
                System.out.println(indexBest + " :" + replaced_pop[indexBest].getFitness() + "  = " + sorted_based_on_fitness[indexBest].getFitness());
                indexBest++;
            } else {
                break;
            }
