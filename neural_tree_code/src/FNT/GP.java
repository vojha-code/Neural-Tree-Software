...
public class GP {
    ...
    /**
     * This is FNT parameter setup function/constructor
     *
     ...
     */
    public GP(MersenneTwisterFast random, EvaluationFunction evaluationFun, ArrayList treeParams, ArrayList gpParameters, ArrayList mhParams, ArrayList algoParams, ArrayList ensembleParameters) {
        m_Random = random;
        ...
        weightsOnly = (boolean) treeParameters.get(8);//int iteration
        ...
        //setting GP parameter
        m_PopulationGP = (int) gpParameters.get(0);//int GP population
        ELITISM = (int) gpParameters.get(1);//double GP Elitism
        MUTATION_PROB = (double) gpParameters.get(2);//double GP Mutation
        CROSSOVER_PROB = (double) gpParameters.get(3);//double GP Crossover
        TOURNAMENT_SIZE = (int) gpParameters.get(4);//int GP Tournament
        m_IterationsGP = (int) gpParameters.get(5);//int GP iteration
        m_Objective_Type = (String) gpParameters.get(6);//String GP training mode/single/multi
    ...
    /**
     * This is FNT main loop function
     *
     * @param inputsCount total number of input columns
     * @param Ot total number of output columns
     * @return
     */
    public FNT[] doEvolution(int inputsCount, int Ot) {
        Pool_Size = (int) (m_PopulationGP * (50 / 100.0));
        FNT m_BestGlobalTree = null;
        FNT m_BestLocalTree = null;
        FNT m_BestWeightsTree = null;
        FNT[] m_BestGlobalTreeArray = new FNT[m_Ensemble_Candidates];
        FNT[] m_BestLocalTreeArray = new FNT[m_Ensemble_Candidates];
        m_BestWeightsTreeArray = new FNT[m_Ensemble_Candidates];//used to compute fitenss hence declered outside
        m_OutputColumn = Ot;
        ...
        try {
            ...
            for (int i = 0; i < m_MaxIterationsGeneral; i++) {
                ...
                FNT[] mainPopulation = randomInitialization(inputsCount);
                ...
                do {
                    ...
                    FNT bestTree = findBestTree(mainPopulation);
                    ...
                    if (m_Objective_Type.equals("Single Objective")) {
                        FNT[] pool = matingPool(mainPopulation, mainPopulation.length);
                        FNT[] offspring = createNewPopulation(pool);
                        offspring = fitnessEvaluation(offspring);
                        System.arraycopy(offspring, 0, mainPopulation, 0, mainPopulation.length);
                        mainPopulation = sortingPopulation(mainPopulation);
                    } else {
                        FNT[] pool = matingPool(mainPopulation, Pool_Size);
                        FNT[] offspring = createNewPopulation(pool);
                        offspring = fitnessEvaluation(offspring);
                        FNT[] intermediatePop = mergePopulation(mainPopulation, offspring);
                ...
                FNT[] tempGlobal = findGLobalBestTree(m_BestGlobalTreeArray);//it murge with previous global and find the best one
    ...
    FNT[] randomInitialization(int inputsCount) {
        FNT[] retTree = new FNT[m_PopulationGP];
        for (int i = 0; i < m_PopulationGP; i++) {
            retTree[i] = new FNT(m_Random);
            do {
            ...
            ////System.out.println(tree.m_FunctChilds.size());
            //retTree.add(tree);
        }
        return retTree;
     ...
     */
    private FNT[] fitnessEvaluation(FNT[] tree) {
        int N = tree.length;
        for (int i = 0; i < N; i++) {
     ...
     */
    private double fitnessEvaluation(FNT tree) {
     ...
     */
    private FNT findBestTree(FNT[] tree) {
        FNT bestTree = null;
        try {
            ...
            ////System.out.println("Best FNT has fitness:"+bestFitness);		
        } catch (Exception e) {
    ...
    private void displayFNTPopulation(FNT[] tree) {
    ...
    private FNT[] sortingPopulation(FNT[] tree) {
        MiscOperations miscOP = new MiscOperations();
        if (m_Objective_Type.equals("Single Objective")) {
    ...
    private FNT[] matingPool(FNT[] tree, int poolSize) {
    ...
    private FNT[] createNewPopulation(FNT[] pool) {
    ...
    FNT[] mergePopulation(FNT[] tree, FNT[] tree1) {
        MiscOperations miscOP = new MiscOperations();
        return miscOP.mergePopulation(tree, tree1);
    }//fun: mergeTree

    private FNT[] reducePopulation(FNT[] tree) {
        MiscOperations miscOP = new MiscOperations();
        if (m_Objective_Type.equals("Single Objective")) {
    ...
    private FNT[] distinctlyReducedPopulaton(FNT[] tree) {
        MiscOperations miscOP = new MiscOperations();
    ...
    private FNT[] findDistinctBest(FNT[] mainPopulation, FNT m_BestLocalTree) {
        //preprocessing before finding best local trees        
        //FNT[] m_bstlclTA = new FNT[m_Ensemble_Candidates];
        if (m_BestLocalTree.getFitness() < findBestTree(mainPopulation).getFitness()) {
            FNT[] diffBest = new FNT[1];
            diffBest[0] = m_BestLocalTree;
        ...
        //return selectManualy(mainPopulation, m_Ensemble_Candidates);
    }//fun : find best local tree
    
    private FNT[] selectManualy(FNT[] mainPopulation, int m_Ensemble_Candidates) {
        //preprocessing before finding best local trees
        int[] indexF = FramesCall.selectIndividuals(mainPopulation, m_Ensemble_Candidates);
        FNT[] m_bestTree = new FNT[m_Ensemble_Candidates];
        for(int i = 0; i <m_Ensemble_Candidates;i++){
            m_bestTree[i] = mainPopulation[indexF[i]].copyTree();
        }
        return m_bestTree;
    }//fun : find best local tree
    
    private void parameterOptimization(FNT[] m_BestLocalTreeArray, boolean weightsOnly) {
        System.out.println(" Parameter Optimization : MH  ");
        TrainingModuleFNT mhAlgo = new TrainingModuleFNT();
        ...
        if (m_Ensemble_Candidates == 1 || m_Ensemble_Diversity.equals("Parametric")) {
            ...
            System.out.println("  " + m_MHAlgo + " dimension : " + mhDimension);
    ...
    private FNT[] findGLobalBestTree(FNT[] globalTreeLocal) {
        MiscOperations miscOP = new MiscOperations();
        FNT[] tree = miscOP.mergePopulation(globalTreeLocal, m_BestWeightsTreeArray);
        return miscOP.replaceWithBestDistinct(tree, m_Ensemble_Candidates);
        /*MiscOperations miscOP = new MiscOperations();
         FNT[] tree = miscOP.mergePopulation(globalTree, m_BestWeightsTreeArray);
